...
### Tests for EDT and UI Update Behavior
...
### UiStyle — Point of Truth for Reusable Constants
...
- Use IntelliJ platform components instead of raw Swing where an equivalent exists.
...
- Avoid hardcoded dimensions. Prefer layout semantics, component defaults, and `JBUI` utilities.
...
- Do not use hardcoded runtime colors in UI code.
- Derive UI colors from semantic IntelliJ theme APIs: `UIUtil`, `JBUI.CurrentTheme`, `NamedColorUtil`, `JBColor.namedColor`, or `JBColor.lazy`.
- Do not use hardcoded font sizes or font families.
...
### Retained Swing Component State
...
Use existing platform utility classes instead of hand-rolling labels, renderers, borders, colors, fonts, and validation behavior.
