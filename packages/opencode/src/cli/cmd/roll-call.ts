	...
	await Instance.provide({
		...
		async fn() {
			...
			if (modelsToTest.length === 0) {
				if (!quiet) UI.println(`${color(UI.Style.TEXT_WARNING)}No models to test after filtering.${color(UI.Style.TEXT_NORMAL)}`)
				return
			}

			if (!quiet) {
				UI.println(`${color(UI.Style.TEXT_INFO)}Prompting ${modelsToTest.length} models...${color(UI.Style.TEXT_NORMAL)}`)
			}
