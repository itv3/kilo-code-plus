...
describe("npm install artifact behavior", () => {
  ...
  test("copies cached binary runtime resources during postinstall", async () => {
    ...
    try {
      ...
      await Bun.write(path.join(bin, "sandbox-mutation-worker.js"), "worker") // kilocode_change
      ...
      expect(await Bun.file(path.join(pkg, "bin", "sandbox-mutation-worker.js")).text()).toBe("worker") // kilocode_change
