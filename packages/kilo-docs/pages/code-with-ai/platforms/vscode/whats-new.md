...
You can now trigger local AI-powered code reviews directly from the CLI using two commands: **`/local-review`** to review all changes on your current branch vs the base branch, and **`/local-review-uncommitted`** to review staged and unstaged changes.
