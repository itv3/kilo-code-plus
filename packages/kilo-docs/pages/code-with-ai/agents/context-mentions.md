...
- **Type `@` for file autocomplete** — In the TUI, type `@` followed by a filename to get autocomplete suggestions. Selecting a file **reads the full file content and injects it into the context window** — it is not just a path reference. For large files this can consume significant context; see the tip below on limiting this with line ranges.
- **Limit context with line ranges** — Append a line range to a file mention (e.g., `@src/utils.ts#10-50`) to include only those lines instead of the entire file. This is the recommended approach for large files when you only need a specific section.
- **Mention file paths in your message** — Simply refer to files by path in your conversation text (e.g., "look at src/utils.ts") without using `@`. The agent will use its `read` tool to fetch only the parts it needs, which is often more token-efficient for large files.
