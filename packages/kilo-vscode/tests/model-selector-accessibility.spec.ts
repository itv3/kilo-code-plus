...
test("model selector exposes combobox relationships and active option movement", async ({ page }) => {
  ...
  await expect(page.locator(".model-selector-group-label").nth(0)).toContainText("NVIDIA")
  await expect(page.locator(".model-selector-group-label").nth(1)).toContainText("Auto Models")
  await expect(page.locator(".model-selector-group-label").nth(2)).toContainText("Recommended")
