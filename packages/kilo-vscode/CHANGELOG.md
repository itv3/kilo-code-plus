...
- 发布批次：`7.3.54-v0.01`，表示基于上游 `7.3.54`，Plus 自定义版本为 `v0.01`。
- 自定义提供商支持 OpenAI / Anthropic / Gemini 三种 API 类型，并支持 Gemini 原生格式。
- 模型自动发现扩展到 OpenAI / Anthropic / Gemini，添加模型时按模型 ID 匹配内置默认模型。
- 匹配到默认模型后，自动带出高级模型参数、图像输入能力、推理能力和成本默认值。
- 支持手动勾选图像输入能力、推理能力，并补齐 context / output token limit 高级配置。
- 支持成本选项，勾选后可配置输入、输出、缓存读取、缓存写入成本。
- 保持最小入侵：主要改动集中在 VS Code 扩展侧，便于后续跟随上游升级。
