...
结论:三项功能均已完成实现并本地打包安装到 Cursor;当前发布批次为 `7.3.54-v0.04`,市场版本为 `7.3.5404`。
...
VS Marketplace / Open VSX 的 `package.json.version` 必须是普通 SemVer 版本,不能使用 `7.3.54-v0.04` 这种带自定义后缀的市场版本。因此 Plus 版本拆成两层:

| 项 | 规则 | 示例 |
|---|---|---|
| 市场版本 | 写入 `packages/kilo-vscode/package.json.version` | `7.3.5404` |
| 发布批次 | 描述基于哪个上游版本和 Plus 自定义版本 | `7.3.54-v0.04` |
| tag | `kilo-code-plus-v<发布批次>` | `kilo-code-plus-v7.3.54-v0.04` |
| GitHub Release | 与 tag 一致 | `kilo-code-plus-v7.3.54-v0.04` |
| VSIX 名称 | `kilo-code-plus-<发布批次>-<target>.vsix` | `kilo-code-plus-7.3.54-v0.04-darwin-arm64.vsix` |

文档表述统一为:基于上游 `7.3.54`,Plus 自定义版本 `v0.04`。
...
1. 确认 `packages/kilo-vscode/package.json.version` 已升级到新的市场版本,例如 `7.3.5404`。
2. 确认 README、开发方案、changeset 已写入本次发布批次,例如 `7.3.54-v0.04`。
3. 新增或更新 `.github/release-notes/kilo-code-plus-v7.3.54-v0.04.md`,逐条列出该批次的具体变更;GitHub Release 正文会优先使用这个文件。
4. 跑最小必要检查:
...
./node_modules/.bin/vsce package --no-dependencies --skip-license --target darwin-arm64 -o out/kilo-code-plus-7.3.54-v0.04-darwin-arm64.vsix
```
...
node esbuild.js --production
./node_modules/.bin/vsce package --no-dependencies --skip-license --target darwin-arm64 -o /tmp/kilo-code-plus-fix/kilo-code-plus-7.3.5404-darwin-arm64.vsix
```
