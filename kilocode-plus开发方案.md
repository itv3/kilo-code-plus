...
2. **功能二: 模型列表过滤、排序与选择体验**:默认仅显示 Kilo Gateway 免费模型以及用户已添加或已连接提供商的模型;聊天窗口中,用户添加或连接的提供商模型排在免费模型前面;切换模型时默认折叠详情页,首次没有收藏记录时默认收藏 StepFun 免费模型。
...
结论:三项功能均已完成实现并本地打包安装到 Cursor;当前发布批次为 `7.3.54-v0.05`,市场版本为 `7.3.5405`。
...
1. 自定义 provider 增强主体放在 `packages/kilo-vscode`;少量 `packages/opencode` 改动仅保留发布构建开关、`env` 删除哨兵、provider 鉴权生命周期调整和必要测试,不改 CLI provider catalog 生成路径。
...
| `packages/kilo-vscode/src/provider-actions.ts` | 保存自定义 provider,并把删除哨兵补丁提交给配置接口 |
| `packages/kilo-vscode/src/shared/custom-provider.ts` | schema、normalize、删除哨兵支持 `modalities` / `limit` / `cost` / `env` / `headers` |
| `packages/kilo-vscode/src/shared/fetch-models.ts` | OpenAI / Anthropic / Gemini 模型发现、endpoint 处理、认证头、价格解析和重定向防护 |
| `packages/kilo-vscode/src/shared/provider-model.ts` | 自定义 provider 包名、发现协议、内置 catalog、baseURL 规范化映射 |
...
`packages/opencode` 触点说明:

| 文件 | 用途 |
|---|---|
| `packages/opencode/src/config/provider.ts` | 允许 `env` 写入 `null`,配合 `stripNulls` 删除旧环境变量配置 |
| `packages/opencode/src/kilocode/server/provider-auth-lifecycle.ts` | 自定义 provider 鉴权变更后只清模型缓存,避免保存 key 时重置全部实例 |
| `packages/opencode/script/build.ts` | 允许 Plus 打包流程构建 release 形态 CLI binary |
| `packages/opencode/test/kilocode/server/provider-auth-lifecycle.test.ts` | 覆盖 provider 鉴权生命周期调整 |
| `packages/opencode/test/provider/provider.test.ts` | 覆盖 provider env 行为 |

`packages/opencode/src/provider/provider.ts` 当前不保留 Plus 行为改动;此前仅包裹上游原逻辑的纯注释标记已回退,避免增加后续 merge 摩擦。
...
聊天窗口的模型选择器默认折叠模型详情页,保持列表优先。折叠状态下沿用现有交互:单击模型行直接切换模型;需要查看价格、上下文、能力等详情时,用户再点击右上角展开按钮。

首次没有收藏记录时,默认收藏 `kilo/stepfun/step-3.7-flash:free`。该默认值只在 `favoriteModels` 未初始化时注入;如果用户手动取消收藏并写入空数组,后续不会自动恢复,避免强制收藏。
...
| `packages/kilo-vscode/src/provider-actions.ts` | 校验收藏模型并在首次无收藏记录时注入 StepFun 默认收藏 |
| `packages/kilo-vscode/webview-ui/src/context/provider-utils.ts` | `isVisibleModel` / `isModelValid` 单一可见性入口 |
| `packages/kilo-vscode/webview-ui/src/components/shared/ModelSelector.tsx` | 主模型选择器按可见性过滤,聊天入口默认折叠详情页 |
...
| `packages/kilo-vscode/tests/unit/provider-actions-validate.test.ts` | 覆盖 StepFun 默认收藏、清空后不恢复和收藏数据清理 |
| `packages/kilo-vscode/tests/model-selector-accessibility.spec.ts` | 覆盖聊天模型选择器默认折叠详情页和模型选择器键盘交互 |
...
VS Marketplace / Open VSX 的 `package.json.version` 必须是普通 SemVer 版本,不能使用 `7.3.54-v0.05` 这种带自定义后缀的市场版本。因此 Plus 版本拆成两层:

| 项 | 规则 | 示例 |
|---|---|---|
| 市场版本 | 写入 `packages/kilo-vscode/package.json.version` | `7.3.5405` |
| 发布批次 | 描述基于哪个上游版本和 Plus 自定义版本 | `7.3.54-v0.05` |
| tag | `kilo-code-plus-v<发布批次>` | `kilo-code-plus-v7.3.54-v0.05` |
| GitHub Release | 与 tag 一致 | `kilo-code-plus-v7.3.54-v0.05` |
| VSIX 名称 | `kilo-code-plus-<发布批次>-<target>.vsix` | `kilo-code-plus-7.3.54-v0.05-darwin-arm64.vsix` |

文档表述统一为:基于上游 `7.3.54`,Plus 自定义版本 `v0.05`。
...
| 当前 targets | `darwin-arm64`、`darwin-x64`、`win32-x64`、`win32-arm64`、`linux-x64`、`linux-arm64` |
| VS Marketplace token | GitHub Secrets: `VSCE_TOKEN`;没有 token 或手动跳过时不阻断 Open VSX / GitHub Release |
| Open VSX token | GitHub Secrets: `OVSX_TOKEN`;兼容保留 `OPENVSX_TOKEN` |
| GitHub Release token | workflow 使用 `github.token` |
| VS Marketplace 跳过参数 | 手动触发 workflow 时默认 `skip_vs_marketplace=true` |

自动发布流程:

1. 解析 tag 或手动输入的发布批次。
2. 读取 `packages/kilo-vscode/package.json.version` 作为市场版本。
3. 构建全平台 CLI binary dist。
4. 用 `packages/kilo-vscode/script/build.ts` 打包平台 VSIX。
5. 整理并重命名 6 个 Plus 发布包。
6. 发布 6 个平台包到 Open VSX。
7. 创建或更新 GitHub Release,并上传 6 个平台 VSIX。

标准发布步骤:

1. 确认 `packages/kilo-vscode/package.json.version` 已升级到新的市场版本,例如 `7.3.5405`。
2. 确认 README、开发方案、changeset 已写入本次发布批次,例如 `7.3.54-v0.05`。
3. 新增或更新 `.github/release-notes/kilo-code-plus-v7.3.54-v0.05.md`,逐条列出该批次的具体变更;GitHub Release 正文会优先使用这个文件。
4. 跑最小必要检查:
...
7. 等待 GitHub Actions 的 `publish-kilo-code-plus` workflow 完成。
8. 确认 GitHub Release 正文包含具体变更,并且有 6 个平台 `.vsix`。
9. 确认 Open VSX 6 个平台下载映射都指向新市场版本。

Open VSX 注意事项:

- `ovsx publish` 返回成功后,Open VSX 后台和公开 API 通常还需要几分钟传播。
- 不要在刚发布成功的几十秒内因为总接口或旧缓存仍显示旧版本就判断失败。
- 验证时以 `targetPlatform` 查询返回的 `downloads` 映射为准,确认 6 个平台都能下载。
- 如果 workflow 日志已经显示 6 行 `Published itv3.kilo-code-plus v<市场版本>@<target>`,优先等待传播,不要立刻修改 workflow 或重复发布。

每次正式发布必须覆盖以下 6 个 target:
...
./node_modules/.bin/vsce package --no-dependencies --skip-license --target darwin-arm64 -o out/kilo-code-plus-7.3.54-v0.05-darwin-arm64.vsix
```
...
node esbuild.js --production
./node_modules/.bin/vsce package --no-dependencies --skip-license --target darwin-arm64 -o /tmp/kilo-code-plus-fix/kilo-code-plus-7.3.5405-darwin-arm64.vsix
```
...
| 扩展版本 | 显示市场版本,例如 `7.3.5405` |
| 关于页面 | 版本信息不再显示 `unknown` |
| 自定义 provider | OpenAI / Anthropic / Gemini 模型发现、保存、图片能力、推理能力、token limit、成本选项正常 |
| 模型列表 | 默认只显示 Kilo 免费模型和用户已连接 provider 模型;聊天切换模型默认折叠详情页;首次无收藏记录时默认收藏 StepFun 免费模型 |
